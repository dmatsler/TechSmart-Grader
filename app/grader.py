...
def _has_four_side_points_with_offset(code: str) -> bool:
    expected = [
        r"\bpoint\w*\s*=\s*\(\s*left\s*,\s*top\s*[-+]\s*\w*offset\w*\s*\)",
        r"\bpoint\w*\s*=\s*\(\s*right\s*,\s*top\s*[-+]\s*\w*offset\w*\s*\)",
        r"\bpoint\w*\s*=\s*\(\s*right\s*,\s*bottom\s*[-+]\s*\w*offset\w*\s*\)",
        r"\bpoint\w*\s*=\s*\(\s*left\s*,\s*bottom\s*[-+]\s*\w*offset\w*\s*\)",
    ]
    return all(regex_any_match([pattern], code) for pattern in expected)
...
def _matches_polygon_side_point_pattern(assignment: AssignmentConfig, zone_name: str, code: str) -> bool:
    ...
    if zone_name == "draw_polygon":
        if not regex_any_match([r"pygame\.draw\.polygon\s*\("], code):
            return False
        point_vars = _extract_point_vars_derived_from_offset(code)
        point_collection_vars = _extract_point_collection_vars(point_vars, code)
        if point_vars and any(_polygon_call_uses_var(code, point_var) for point_var in point_vars):
            return True
        if point_collection_vars and any(_polygon_call_uses_var(code, collection_var) for collection_var in point_collection_vars):
            return True
        return _has_four_side_points_with_offset(code)
    ...
    for zone in assignment.fill_zones:
        ...
        if assignment.id == TARGET_ASSIGNMENT_POLYGON_PATTERN and zone.name == "polygon_points":
            ...
            matched_attempt_line = matched_attempt_line or regex_any_match(
                [r"\bpoint\w*\s*=\s*\(.*\)", r"\b(left|right|top|bottom)\b"],
                code,
            )
